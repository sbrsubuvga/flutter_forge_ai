...
class CliLogger {
  ...
  String _wrap(String text, String code) => _ansi ? '$code$text$_reset' : text;

  /// Prints an info line.
  void info(String message) => stdout.writeln(message);

  /// Prints a success ✓ line in green.
  void success(String message) => stdout.writeln(_wrap('✓ $message', _green));

  /// Prints a warning ⚠ line in yellow.
  void warning(String message) => stdout.writeln(_wrap('⚠ $message', _yellow));

  /// Prints an error ✗ line in red.
  void error(String message) => stderr.writeln(_wrap('✗ $message', _red));

  /// Prints a blue [hint] (used by `doctor` for fix suggestions).
  void hint(String message) => stdout.writeln(_wrap('  → $message', _blue));
